...
export function SetRow({ setNumber, weightKg, reps, rpe, isPB, onDelete }: SetRowProps) {
  return (
    <div className={`flex items-center justify-between py-2 px-3 rounded-lg ${isPB ? "bg-yellow-500/10 border border-yellow-500/30" : "bg-secondary/50"}`}>
      <div className="flex items-center gap-4">
        ...
        {isPB && <Trophy className="h-3.5 w-3.5 text-yellow-500" />}
      </div>
      {onDelete && (
        <button
          onClick={onDelete}
